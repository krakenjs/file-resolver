...
test('fileResolver', function (t) {
    ...
    t.test('Creating a file resolver with options', function (t) {
        var resolvr = fileResolver.create({root: path.resolve(__dirname, 'fixtures/root'), fallback: 'en_US', ext: 'dust'});
    ...
    t.test('resolving for an extension with default locale', function (t) {
        ...
        t.equal(path.resolve(info.root), path.resolve(__dirname, 'fixtures/root/US/en'));
        t.equal(info.file, path.resolve(__dirname, 'fixtures/root/US/en/test.dust'));
    ...
    t.test('resolving for an extension with a specified locale', function (t) {
        ...
        t.equal(path.resolve(info.root), path.resolve(__dirname, 'fixtures/root/US/es'));
        t.equal(info.file, path.resolve(__dirname, 'fixtures/root/US/es/test.dust'));
        t.equal(info.ext, 'dust');
        t.equal(info.name, 'test');
        t.end();
    });
    ...
    t.test('Resolve a bundle not in the primary langauge but is in the fallback', function (t) {
        ...
        t.equal(path.resolve(info.root), path.resolve(__dirname, 'fixtures/root/US/en'));
        t.equal(info.file, path.resolve(__dirname, 'fixtures/root/US/en/fallback.dust'));
        t.equal(info.ext, 'dust');
    ...
    t.test('Creating a file resolver with options', function (t) {
        var resolvr = fileResolver.create({root: path.resolve(__dirname, 'fixtures/root'), ext: 'dust'});
    ...
    t.test('resolving for an extension without locale', function (t) {
        ...
        t.equal(path.resolve(info.root), path.resolve(__dirname, 'fixtures/root'));
        t.equal(info.file, path.resolve(__dirname, 'fixtures/root/test.dust'));
        t.equal(info.ext, 'dust');
        t.equal(info.name, 'test');
        t.end();
    });
    ...
    t.test('Creating a file resolver with invalid root', function (t) {
        var resolvr = fileResolver.create({root: path.resolve(__dirname, 'fixtures'), ext: 'dust'});
        t.deepEqual(resolvr.fallbackLocale, { country: '', language: '' });
        t.equal(typeof resolvr._locate, 'function');
        var info = resolvr.resolve('test');
    ...
    t.test('Creating a file resolver with invalid locale', function (t) {
        var resolvr = fileResolver.create({root: path.resolve(__dirname, 'fixtures/root'), ext: 'dust', fallback: 'es'});
